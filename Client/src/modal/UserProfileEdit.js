...
export default function UserProfileEdit({ handleClose, open, user }) {
  ...
  const [avatarSrc, setAvatarSrc] = React.useState(user?.profile);
          ...
          <Box display={"flex"} gap={2} flexDirection={{ xs: "column" }}>
            ...
            <Box display="flex" justifyContent="center">
              <Button color="primary" variant="contained" onClick={handleSave}>
                {state.loading ? <BeatLoader color="white" /> : "Save"}
              </Button>
            </Box>
          </Box>
        </Box>
